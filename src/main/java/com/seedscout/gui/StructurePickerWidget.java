...
public final class StructurePickerWidget extends ObjectSelectionList<StructurePickerWidget.Entry> {
    ...
    public StructurePickerWidget(Minecraft client, int x, int y, int width, int height, List<Identifier> all,
                                 boolean biomes, Consumer<Identifier> onPick) {
    ...
    public final class Entry extends ObjectSelectionList.Entry<Entry> {
        ...
        public void extractContent(GuiGraphicsExtractor context, int mouseX, int mouseY, boolean hovered, float deltaTicks) {
            ...
            int y = getY() + 2;
            if (biomes) {
                StructureIcons.drawSwatch(context, BiomeColors.colorOf(id), x, y);
            } else {
